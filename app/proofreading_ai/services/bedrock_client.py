...
**🟡 人名校正（新機能v2）**：
政治家：岸田首相→岸田文雄首相、菅前首相→菅義偉前首相
芸能人：さんま→明石家さんま、タモリ→タモリ、ダウンタウン松本→松本人志
スポーツ選手：大谷→大谷翔平、藤井→藤井聡太（将棋の文脈で）
        ...
        try:
            ...
            if "✅修正箇所：" in response_text:
                ...
                for line in lines:
                    ...
                    # 新しい形式対応: - 行番号: (変更前) -> (変更後): 理由 [カテゴリー: expression|typo|dictionary|contradiction]
                    if line.startswith('- ') and ': ' in line and '[カテゴリー:' in line:
                        try:
                            ...
                            # カテゴリー部分を抽出
                            category_match = re.search(r'\[カテゴリー:\s*(expression|typo|dictionary|contradiction)\]', content_part)
                            category = 'general'
                            if category_match:
